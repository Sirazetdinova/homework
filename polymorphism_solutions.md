...
Утиная типизация - прием в программировании, который можно описать следующим образом: если что-то выглядит как утка и ведет себя как утка, это переменная типа «утка». 
Хотя класс `Mouse` имеет нужный нам метод `feed()`, с точки зрения номинальной типизации он не является подтипом `Bird`.
...
class Rectangle(GeometricFigures):
    ...
    def get_perimeter(self):
        return f'Периметр равен: {self.a + self.b}'  
```
