...
1. **Тест инициализации задачи `test_task_initialization`**:
    ...
    - Убедиться, что приоритет равен 1, дедлайн не установлен None), и статус выполнения "Не выполнено" и задача не помечена как завершенная.
...
6. **Тест удаления задач из списка `test_remove_task`**:
    ...
    - Убедиться, что после удаления задачи, количество задач в списке `task_list` уменьшается на 1.
...
7. **Тест сортировки задач по приоритету `test_sort_by_priority`:**
    ...
    - Вызывать метод `sort_by_priority` для сортировки задач в списке `task_list`.
...
8. **Тест фильтрации задач по статусу выполнения `test_filter_by_status`:**
    ...
    - Выполнить фильтрацию задач по статусу выполнения "Выполнено" с помощью функции `filter_by_status`.
...
1. Запустите программу Automatic Watering Systemr.
2. Проверьте каждый тестовый случай, описанный ниже, с помощью автоматизированных тестов.
3. Запишите результаты тестов и заметки об ошибках, если они есть. 
...
1. **Тест запуска полива с достаточным количеством воды `test_start_watering_with_enough_water`:**
   ...
   - Вызовите метод start_watering с параметрами duration_minutes=10 и water_amount, который определен в параметризованной фикстуре. Этот шаг выполняется дважды: с water_amount=100 и water_amount=200.
   - Убедиться, что после вызова метода start_watering начался полив, атрибут is_watering объекта AutomaticWateringSystem равен True.
...
2. **Тест запуска полива с недостаточным количеством воды `test_start_watering_with_insufficient_water`:**
  ...
  - Убедиться, что после вызова метода start_watering не начался полив, атрибут is_watering объекта AutomaticWateringSystem равен False.
...
- Убедиться, что расписание установлено правильно, т. е. атрибут schedule содержит ожидаемые значения словаря
